...
`src/input/policy.ts`の対応一覧をengineの候補選択、割り当て検証、テストの基準にします。部分一致・完全一致の両方を制限し、マーク・ジャンプリスト・Exなどの非対応操作を間接再生からも実行しません。文字引数、Insert文字列、検索語はコマンドと区別します。不成立のキー列は待機を解除して終了し、接頭辞や後続文字をホストへ再送しません。非対応の割り当ては保存内容を保持して適用だけを停止します。
